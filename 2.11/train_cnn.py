"""
Baby Cry Classification - CRNN Training Script (Mel Spectrogram Only)
=====================================================================
3.3/train.py의 CRNN 아키텍처를 기반으로,
기존 전처리된 npy mel spectrogram 데이터로 5클래스 분류.
...
def parse_args():
    parser = argparse.ArgumentParser(description='Baby Cry CRNN Training (5 Classes)')
...
class MelDataset(Dataset):
    ...
    def __getitem__(self, idx):
        mel = self.mel[idx]  # (128, 299)

        if self.augment:
            # 시간 마스킹 (SpecAugment 스타일)
        ...
        mel = mel.unsqueeze(0)  # (1, 128, 299) — 채널 추가
        return mel, self.labels[idx]


# =============================================================================
# 4. CRNN 모델 (3.3/train.py 기반)
# =============================================================================

class BabyCryCRNN(nn.Module):
    """
    CNN → BiLSTM → Classifier
    3.3/train.py의 DeepInfantModel 아키텍처를 기반으로 수정.
    """
    def __init__(self, num_classes=5):
        ...
        # 채널=256, freq=16, time=37
        self.lstm_input_size = 256 * 16  # channels * freq_bins
    ...
    def forward(self, x):
        ...
        x = x.reshape(batch_size, x.size(1), -1)  # (batch, 37, 256*16=4096)

        # BiLSTM
        x, _ = self.lstm(x)  # (batch, 37, 512)
